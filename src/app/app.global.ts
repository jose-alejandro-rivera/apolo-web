...
export class AppGlobals {
    ...
    readonly mensajeCertificacionRealizada: string = ' ¿Ha realizado la certificación del servicio?';
    readonly mensajeNoEvidenciaToa: string = ' No se evidencia en TOA el resultado ok de la certificación de servicio.';
    readonly mensajecertificacionAhora: string = ' ¿Desea que se realice la certificación de servicio ahora?';
    readonly mensajeAseguraCertificacion: string = ' Debe asegurar la certificación de servicio correcta para continuar en el flujo';
    ...
    public validateIP() {
        ...
        if (urlIp.indexOf("http://10.203.222.125") > -1) {
            this.url = 'http://10.203.222.125:8080/api/'
        }
        if (urlIp.indexOf("http://10.203.222.126") > -1) {
            this.url = 'http://10.203.222.126:8080/api/'
        } 
       /*if (urlIp.indexOf("http://10.203.222.125") > -1) {
            this.url = 'http://10.203.222.125/api/'
        }
        if (urlIp.indexOf("http://10.203.222.126") > -1) {
            this.url = 'http://10.203.222.126/api/'
        }  */     
